...
from networks.PMQM.popu_memory_retrive import PopuMemoryNet


class ConditionGenMSTFCMPopuMemoryRetrive(nn.Module):
    """ condition generator """
    def __init__(self,d_model, start_memory_step=1500, dropout=0.1):
        super(ConditionGenMSTFCMPopuMemoryRetrive, self).__init__()
        ...
        self.seq_emb = ImgTimeLabelEmbeddingModule(**embd_config)
        self.mstformer = MultiscaleSpatialTemporalTransformer(**mstformer_config)
        self.pmn = PopuMemoryNet(**pmn_config)
    ...
    def get_cp(self,ch, xt_seq_embd, t_seq, call_assign=True):
        """
        ...
        cp, assign_prob = self.pmn(ch=ch, xt_seq_embd=xt_seq_embd, t_seq=t_seq, call_assign=call_assign)
        return cp, assign_prob
    ...
    def forward(self, x_seq, t_seq, l_seq, current_step=None):
        ...
        if current_step is None:  # evaluation
            ...
            cp, assign_prob = self.get_cp(ch=ch, xt_seq_embd=xt_seq_embd, t_seq=t_seq)
            return ch, cp, assign_prob
            ...
            if current_step <= self.start_memory_step: # train ch first to get stable query, don't use PMN
                ...
                cp,_ = self.get_cp(ch=ch, xt_seq_embd=xt_seq_embd, t_seq=t_seq)
                return ch, cp
...
if __name__ == '__main__':
    ...
    model = ConditionGenMSTFCMPopuMemoryRetrive(d_model=32).to(device)
    # training
    # model.train()
    # ch,cp = model(x_seq, t_seq, l_seq, current_step=1000)
    # evaluation
    model.eval()
    ch,cp, assign_prob = model(x_seq, t_seq, l_seq)
    print(ch.shape)